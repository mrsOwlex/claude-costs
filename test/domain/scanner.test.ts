...
import { chooseRequestEntry, tokensEqual, totalRequestTokens, deduplicateAndAggregate } from '../../src/domain/scanner.js';
import type { Request } from '../../src/contracts/request.js';
import type { TokenBucket } from '../../src/contracts/tokens.js';

function makeTokens(input: number, output = 0, cacheRead = 0, cacheCreateTotal = 0): TokenBucket {
  return {
    input,
    output,
    cacheRead,
...
function makeReq(key: string, input: number, timestamp: string | null = null): Request {
  return {
    key,
    messageId: null,
    requestId: null,
...
test('totalRequestTokens sums all four fields', () => {
  const req = { tokens: makeTokens(10, 5, 3, 2) } as Request;
...
test('deduplicateAndAggregate skips synthetic models', () => {
  const map1 = new Map<string, Request>([
...
test('deduplicateAndAggregate aggregates by model', () => {
  const req1: Request = { ...makeReq('k1', 10, '2026-05-01T10:00:00Z'), model: 'claude-opus-4-6' };
  const req2: Request = { ...makeReq('k2', 5, '2026-05-01T11:00:00Z'), model: 'claude-haiku-4-5' };
  const map1 = new Map<string, Request>([['k1', req1], ['k2', req2]]);
  const result = deduplicateAndAggregate([map1]);
