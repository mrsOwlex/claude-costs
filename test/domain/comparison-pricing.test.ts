...
import { calculateComparisonTraceCost, calculateComparisonNoCacheTraceCost, calculateAgenticAdjustedRange } from '../../src/domain/comparison-pricing.js';
...
const TOKENS: TokenBucket = { input: 100, output: 10, cacheRead: 1_000, cacheCreate5m: 500, cacheCreate1h: 1_500, cacheCreateUnknown: 0, cacheCreate: 2_000, cacheCreateTotal: 2_000 };
const REQUEST = { model: 'test', tokens: TOKENS };

test('provider cache mode uses both cache read and write prices', () => {
  const model: ComparisonModel = { id: 'full', name: 'Full Cache', input: 1e-6, output: 10e-6, cacheRead: 0.1e-6, cacheCreate: 0.5e-6 };
...
test('partial cache mode prices reads but bills writes as input', () => {
  const model: ComparisonModel = { id: 'partial', name: 'Partial', input: 1e-6, output: 10e-6, cacheRead: 0.1e-6, cacheCreate: null };
  const cost = calculateComparisonTraceCost(model, [REQUEST]);
...
test('no-cache mode bills everything as regular input', () => {
  const model: ComparisonModel = { id: 'full', name: 'Full', input: 1e-6, output: 10e-6, cacheRead: 0.1e-6, cacheCreate: 0.5e-6 };
...
test('model without any cache prices collapses cache into input', () => {
  const model: ComparisonModel = { id: 'none', name: 'No Cache', input: 1e-6, output: 10e-6, cacheRead: null, cacheCreate: null };
  const cost = calculateComparisonTraceCost(model, [REQUEST]);
...
test('context limit exceeded emits warning', () => {
  const model: ComparisonModel = { id: 'small', name: 'Small', input: 1e-6, output: 10e-6, cacheRead: null, cacheCreate: null, contextLength: 500 };
  const cost = calculateComparisonTraceCost(model, [REQUEST]);
...
test('output limit exceeded emits warning', () => {
  const model: ComparisonModel = { id: 'limited', name: 'Limited', input: 1e-6, output: 10e-6, cacheRead: null, cacheCreate: null, maxOutputTokens: 5 };
  const cost = calculateComparisonTraceCost(model, [REQUEST]);
...
test('agentic range multiplier preserves original cost', () => {
  const traceCost = { total: 100 } as ComparisonTraceCostResult;
  const model = {} as ComparisonModel;
  const range = calculateAgenticAdjustedRange(model, traceCost, { min: 2, max: 5 });
...
test('agentic range defaults to 1x-3x', () => {
  const range = calculateAgenticAdjustedRange({} as ComparisonModel, { total: 10 } as ComparisonTraceCostResult);
