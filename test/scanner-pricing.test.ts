...
} from '../src/domain/claude-pricing.js';
import {
  calculateAgenticAdjustedRange,
  calculateComparisonNoCacheTraceCost,
  calculateComparisonTraceCost,
} from '../src/domain/comparison-pricing.js';
import type { ComparisonTraceCostResult } from '../src/domain/comparison-pricing.js';
import type { ComparisonModel } from '../src/contracts/pricing-model.js';
import { buildComparisons } from '../src/application/build-comparisons.js';
import type { CLIArgs } from '../src/adapters/driving/cli.js';
...
test('Claude request cost uses 1h cache write pricing', () => {
  const cost = calculateClaudeRequestCost({
    ...
    tokens: {
      ...
      cacheCreate5m: 0, cacheCreate1h: 1_000_000, cacheCreateUnknown: 0,
      cacheCreate: 1_000_000, cacheCreateTotal: 1_000_000,
    },
  }, CLAUDE_PRICING);
...
test('Sonnet 4.5 long-context premium is applied per request', () => {
  const trace = calculateClaudeTraceCost([
    { key: 'r1', messageId: null, requestId: null, timestamp: null, date: null, model: 'claude-sonnet-4-5', tokens: { input: 199_000, output: 0, cacheRead: 0, cacheCreate5m: 0, cacheCreate1h: 0, cacheCreateUnknown: 0, cacheCreate: 0, cacheCreateTotal: 0 } },
    { key: 'r2', messageId: null, requestId: null, timestamp: null, date: null, model: 'claude-sonnet-4-5', tokens: { input: 201_000, output: 0, cacheRead: 0, cacheCreate5m: 0, cacheCreate1h: 0, cacheCreateUnknown: 0, cacheCreate: 0, cacheCreateTotal: 0 } },
  ], CLAUDE_PRICING);
...
test('Sonnet 4.6 does not receive 200K long-context premium', () => {
  const cost = calculateClaudeRequestCost({
    ...
    tokens: { input: 250_000, output: 0, cacheRead: 0, cacheCreate5m: 0, cacheCreate1h: 0, cacheCreateUnknown: 0, cacheCreate: 0, cacheCreateTotal: 0 },
  }, CLAUDE_PRICING);
...
test('comparison models without cache collapse cache tokens into regular input', () => {
  const cost = calculateComparisonTraceCost(
    ...
    [{
      tokens: { input: 100, output: 10, cacheRead: 1_000, cacheCreate5m: 500, cacheCreate1h: 1_500, cacheCreateUnknown: 0, cacheCreate: 2_000, cacheCreateTotal: 2_000 },
    }],
  );
...
test('context limit warnings do not invalidate comparison cost', () => {
  const cost = calculateComparisonTraceCost(
    ...
    [{
      tokens: { input: 100, output: 10, cacheRead: 1_000, cacheCreate5m: 0, cacheCreate1h: 0, cacheCreateUnknown: 0, cacheCreate: 0, cacheCreateTotal: 0 },
    }],
  );
...
test('partial cache pricing remains distinct from no-cache scenario', () => {
  const model: ComparisonModel = { id: 'partial', name: 'Partial Cache', input: 1 / 1_000_000, output: 10 / 1_000_000, cacheRead: 0.1 / 1_000_000, cacheCreate: null };
  const requests = [{
    tokens: { input: 100, output: 10, cacheRead: 1_000, cacheCreate5m: 500, cacheCreate1h: 1_500, cacheCreateUnknown: 0, cacheCreate: 2_000, cacheCreateTotal: 2_000 },
  }];
...
test('comparison builder exposes no-cache monthly cost and keeps context-limited models', () => {
  const { comparisons } = buildComparisons(
    ...
    [{
      key: 'r1', messageId: null, requestId: null, timestamp: null, date: null,
      model: 'claude-opus-4-6',
      tokens: { input: 100, output: 10, cacheRead: 1_000, cacheCreate5m: 500, cacheCreate1h: 1_500, cacheCreateUnknown: 0, cacheCreate: 2_000, cacheCreateTotal: 2_000 },
    }],
    { agenticMultiplier: { min: 1, max: 3 }, comparison: 'both', budget: 100 } as CLIArgs,
    1,
  );
...
test('agentic multiplier changes only the scenario range, not same-trace cost', () => {
  const traceCost = { total: 10 } as ComparisonTraceCostResult;
  const range = calculateAgenticAdjustedRange(
    { id: 'model', name: 'Model' } as ComparisonModel,
    traceCost,
    { min: 1.5, max: 2.5 },
  );
