...
const COMPARISON_MODELS = [
  // OpenAI
  { id: 'openai/gpt-5.5', name: 'GPT-5.5', input: perM(5.00), output: perM(30.00), cacheRead: null },
  { id: 'openai/gpt-5.5-pro', name: 'GPT-5.5 Pro', input: perM(30.00), output: perM(180.00), cacheRead: null },
  { id: 'openai/gpt-5.4', name: 'GPT-5.4', input: perM(2.50), output: perM(15.00), cacheRead: perM(0.25) },
  { id: 'openai/gpt-5.4-mini', name: 'GPT-5.4 Mini', input: perM(0.75), output: perM(4.50), cacheRead: perM(0.075) },
  { id: 'openai/gpt-5.4-nano', name: 'GPT-5.4 Nano', input: perM(0.20), output: perM(1.25), cacheRead: perM(0.02) },
  // Google
  { id: 'google/gemini-3.5-flash', name: 'Gemini 3.5 Flash', input: perM(1.50), output: perM(9.00), cacheRead: null },
  { id: 'google/gemini-3.1-flash-lite', name: 'Gemini 3.1 Flash Lite', input: perM(0.25), output: perM(1.50), cacheRead: null },
  // DeepSeek
  { id: 'deepseek/deepseek-v4-pro', name: 'DeepSeek V4 Pro', input: perM(0.435), output: perM(0.87), cacheRead: perM(0.004) },
  { id: 'deepseek/deepseek-v4-flash', name: 'DeepSeek V4 Flash', input: perM(0.10), output: perM(0.20), cacheRead: perM(0.02) },
  // Qwen
  { id: 'qwen/qwen3.7-max', name: 'Qwen 3.7 Max', input: perM(2.50), output: perM(7.50), cacheRead: null },
  { id: 'qwen/qwen3.6-max-preview', name: 'Qwen 3.6 Max', input: perM(1.04), output: perM(6.24), cacheRead: null },
  // Kimi
  { id: 'moonshotai/kimi-k2.6', name: 'Kimi K2.6', input: perM(0.73), output: perM(3.49), cacheRead: perM(0.25) },
  // Anthropic via OpenRouter
  { id: 'anthropic/claude-opus-4.7', name: 'Claude Opus 4.7 (API)', input: perM(5.00), output: perM(25.00), cacheRead: perM(0.50) },
  { id: 'anthropic/claude-sonnet-4.6', name: 'Claude Sonnet 4.6 (API)', input: perM(3.00), output: perM(15.00), cacheRead: perM(0.30) },
  { id: 'anthropic/claude-haiku-4.5', name: 'Claude Haiku 4.5 (API)', input: perM(1.00), output: perM(5.00), cacheRead: perM(0.10) },
];
...
export function calculateComparisonCost(compModel, inputTokens, outputTokens, cacheReadTokens, cacheCreateTokens) {
  // Models without caching must process all cache tokens as regular input
  const effectiveInput = compModel.cacheRead != null
    ? inputTokens
    : inputTokens + cacheReadTokens + cacheCreateTokens;

  const effectiveCacheRead = compModel.cacheRead != null ? cacheReadTokens : 0;

  const inputCost = effectiveInput * compModel.input;
  const outputCost = outputTokens * compModel.output;
  const cacheReadCost = effectiveCacheRead * (compModel.cacheRead || 0);
  ...
  return {
    ...
    hasCache: compModel.cacheRead != null,
  };
}
